...
export class TemplateAnalysisService {
  ...
  async analyzeTemplateImage(
    ...
    if (!this.openai) {
      return {
        ...
        formSchema: {
          ...
          textFields: [],
          tips: [
            'Configure OPENAI_API_KEY to enable automatic template analysis.',
          ],
        },
      };
    }
    ...
    if (!parsed?.reconstructionPrompt || !parsed?.reconstructionSpec) {
      ...
      return {
        reconstructionPrompt:
          text || 'Failed to parse reconstruction JSON from model.',
...
async function refineReconstructionSpecTextBlockExtentsFromPixels(args: {
  ...
  if (!textRegions || typeof textRegions !== 'object')
  ...
  const bottomColorHex = String(
    regions?.[regions.length - 1]?.fillColor ?? '',
  ).trim();
...
function isNearFillColor(
  ...
  if (isTargetRed)
    return (
      ...
      px.g <= 120 &&
      px.b <= 120 &&
      px.r - Math.max(px.g, px.b) >= 40
    );

  // Generic RGB distance (Manhattan) with a loose threshold to handle compression.
  const dist =
    Math.abs(px.r - target.r) +
    Math.abs(px.g - target.g) +
    Math.abs(px.b - target.b);
  return dist <= 160;
}
