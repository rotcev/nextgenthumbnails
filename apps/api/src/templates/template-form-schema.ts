...
export function deriveFormSchemaFromTemplateConfig(
  config: TemplateConfigLike,
): TemplateFormSchemaV1 {
  const subjectSlots = Array.isArray(config?.subjectSlots)
    ? config.subjectSlots
    : [];
  const textRegions = Array.isArray(config?.textRegions)
    ? config.textRegions
    : [];
    ...
    textFields: textRegions
      .map((t) => {
        ...
        return {
          ...
          required,
          placeholder,
        } satisfies TemplateFormTextField;
      })
...
export function coerceFormSchemaForTemplate(
  ...
  const subjectSlots = derived.subjectSlots.map((s, i) => {
    ...
    const helpText =
      typeof src?.helpText === 'string' ? src.helpText : undefined;
  ...
  const textFields = derived.textFields.map((t, i) => {
    ...
    const helpText =
      typeof src?.helpText === 'string' ? src.helpText : undefined;
    const placeholder =
      typeof src?.placeholder === 'string' ? src.placeholder : t.placeholder;
    const defaultValue =
      typeof src?.defaultValue === 'string' ? src.defaultValue : undefined;
    const styleRaw = src?.style;
    const style =
      styleRaw && typeof styleRaw === 'object'
        ? {
            fillColor:
              typeof styleRaw?.fillColor === 'string'
                ? styleRaw.fillColor
                : undefined,
            gradient:
              typeof styleRaw?.gradient === 'string'
                ? styleRaw.gradient
                : undefined,
            innerBorder:
              typeof styleRaw?.innerBorder === 'boolean'
                ? styleRaw.innerBorder
                : undefined,
          }
        : undefined;
  ...
  const customizations = coerceCustomizations(
    (formSchema as any).customizations,
  );
...
function coerceCustomizations(
  value: unknown,
): TemplateFormCustomization[] | null {
  ...
  for (const raw of value) {
    ...
    const helpText =
      typeof raw.helpText === 'string' ? raw.helpText : undefined;

    if (kind === 'toggle') {
      const defaultValue =
        typeof raw.defaultValue === 'boolean' ? raw.defaultValue : undefined;
      const onInstruction =
        typeof raw.onInstruction === 'string' ? raw.onInstruction : undefined;
      out.push({
        id,
        label,
        helpText,
